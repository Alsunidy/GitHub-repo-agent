...
٦١ اختباراً، وتعمل **بلا مفاتيح API وبلا شبكة** — كل اتصال خارجي
...
من البيئة — نفس ٦١ نجاح.

| الملف | يغطي |
|---|---|
| `test_github_tools.py` | ٨ أشكال روابط صالحة و٦ غير صالحة، `RepoNotFound` عند 404، `MissingToken` بلا رمز |
| `test_osv_tools.py` | `parse_requirements` يتجاهل `>=`/`~=`/`-e`/`git+`/التعليقات؛ فشل حزمة واحدة لا يوقف الباقي؛ `fixed_version` يأخذ أعلى إصلاح عبر كل الثغرات، ويتجاهل هاشات git، ويبقى `None` بلا بيانات إصلاح من النظام البيئي |
| `test_secret_tools.py` | كل الأنماط الخمسة للأسرار، التقنيع الصحيح، صفر إشارات كاذبة |
| `test_agents.py` | لكل وكيل: شكل الملاحظة، مسار النموذج، والمسار البديل؛ `fixed_version` يرافق ملاحظات الثغرات ولا يرافق الأسرار أبداً؛ أرقام البلاغات اللي يخترعها النموذج تُسقَط |
| `test_report.py` | ترتيب الخطورة قبل أي استدعاء للنموذج؛ "الكل نظيف" و"لا ملاحظات" ينتجان تقريراً صالحاً **بلا استدعاء للـ LLM أبداً** (تأكيد صريح يفشل لو استُدعي)؛ سطر الترقية بنصه الدقيق باللغتين، وبقاؤه رغم فشل النموذج، وعدم تكراره من توصية النموذج |
...
tests/                  ٦١ اختباراً، بلا مفاتيح، بلا شبكة، بالإضافة للإثباتات المحفوظة
